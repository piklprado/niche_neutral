...
m.niche <- function(ab, spp, trait, grad, family, site, ...){
      es <- glmer(ab ~ trait + I(grad^2) + (1|spp) + (1|site), family=family, 
               control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
      es.tradeoff <- glmer(ab ~ trait + I(grad^2) + trait:grad + (1|spp) + (1|site),
                         family=family, 
                         control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
      es <- glmer(ab ~ trait + I(grad^2) + (1|spp) + (1|site), family=family, 
                  control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
      ## Models to check wrong traits
      es.wt <- glmer(ab ~ grad + I(grad^2) + (1|spp) + (1|site), family=family, 
               control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
      es.tradeoff.wt <- glmer(ab ~ grad + I(grad^2) + (grad|spp) + (1|spp) + (1|site),
                         family=family, 
                    control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))
      return(list(es=es, es.tradeoff=es.tradeoff, es.wt=es.wt, es.tradeoff.wt=es.tradeoff.wt))
}
...
m.neu <- function(ab, spp, region, site, family, ...){
    drift <- glmer(ab ~ 1 + (1|spp:region) + (1|spp) + (1|site), family=family, 
                   control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
    ## site-level dispersal limitation
    drift2 <- glmer(ab ~ 1 + (1|spp:region) + (1|spp:site) + (1|spp) + (1:site), family=family, 
                   control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
    return(list(drift=drift, drift2=drift2))
}
...
m.nineu <- function(ab, trait, grad, spp, region, site, family, ...){
    es.drift <- glmer(ab ~ trait + I(grad^2) + (1|spp:region) + (1|spp) + (1|site), family=family, 
                      nAGQ=1, control=glmerControl(optimizer="bobyqa", 
                                              optCtrl=list(maxfun=2e5)))
    es.tradeoff.drift <- glmer(ab ~ trait + I(grad^2) + trait:grad + 
                                  (1|spp:region) + (1|spp) + (1|site), family=family, 
                                control=glmerControl(optimizer="bobyqa", 
                                                     optCtrl=list(maxfun=2e5)))
    ## With site-level diespersion limitation
    es.drift.2 <- glmer(ab ~ trait + I(grad^2) + (1|spp:region) + (1|spp:site) + (1|spp) + (1|site), family=family, 
                      nAGQ=1, control=glmerControl(optimizer="bobyqa", 
                                              optCtrl=list(maxfun=2e5)))
    es.tradeoff.drift.2 <- glmer(ab ~ trait + I(grad^2) + trait:grad + 
                                  (1|spp:region) + + (1|spp:site) + (1|spp) + (1|site), family=family, 
                                control=glmerControl(optimizer="bobyqa", 
                                                     optCtrl=list(maxfun=2e5)))
    ## wrong traits
    es.drift.wt <- glmer(ab ~ grad + I(grad^2) + (grad|spp) + (1|spp:region) + (1|spp) + (1|site), family=family, 
                      nAGQ=1, control=glmerControl(optimizer="bobyqa", 
                                              optCtrl=list(maxfun=2e5)))
    es.tradeoff.drift.wt <- glmer(ab ~ grad + I(grad^2) + (grad|spp) + 
                                  (1|spp:region) + (1|spp) + (1|site), family=family, 
                                control=glmerControl(optimizer="bobyqa", 
                                                     optCtrl=list(maxfun=2e5)))
    ## Site-level dispersal limitation
    es.drift.wt.2 <- glmer(ab ~ grad + I(grad^2) + (grad|spp) + (1|spp:region) + (1|spp:site) + (1|spp) + (1|site), family=family, 
                      nAGQ=1, control=glmerControl(optimizer="bobyqa", 
                                              optCtrl=list(maxfun=2e5)))
    es.tradeoff.drift.wt.2 <- glmer(ab ~ grad + I(grad^2) + (grad|spp) + 
                                  (1|spp:region) + (1|spp:site) + (1|spp) + (1|site), family=family, 
                                control=glmerControl(optimizer="bobyqa", 
    return(list(es.drift=es.drift, es.tradeoff.drift=es.tradeoff.drift,
                es.drift.2=es.drift.2, es.tradeoff.drift.2=es.tradeoff.drift.2,
                es.drift.wt=es.drift.wt, es.tradeoff.drift.wt=es.tradeoff.drift.wt,
                es.drift.wt.2=es.drift.wt.2, es.tradeoff.drift.wt.2=es.tradeoff.drift.wt.2
                )) 
}

## 4. Null hypothesis
## Null model in which species and sites are independent random intercepts
m.null <- function(ab, region, site, spp, family){
      mnull <- glmer(ab ~ 1 + (1|region) + (1|spp) + (1|site), family=family,
                   control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=5e5)))
      return(list(null=mnull))
}
```
...
neu.det <- m.neu(ab=all.data$det, 
                      ...
                      site=all.data$site, ## estava site=all.data$grad. Erro?
                      "poisson")
...
null.det <- m.null(ab=all.data$det, 
                   ...
                   "poisson")
## BIC for each model
lista1 <- c(null.det,neu.det, niche.det.rt, nineu.det.rt)
BICtab( lista1, nobs=nrow(all.data), base=TRUE)

mod.det.rt.BIC <- c(
    sapply(null.det, BIC),
    sapply(neu.det, BIC),
    sapply(niche.det.rt, BIC),
    sapply(nineu.det.rt, BIC)
    )
