...
<!-- NÃO GOSTO MUITO DO TITULO POR STEP BY STEP, INCLUSIVE PARA O NOME DO ARQUIVO. SUGIRO ALGO COMO "MIXED MODELS FOR COMMUNITY ASSEMBLY TUTORIAL" E O ARQUIVO COMO GLMMcommunity.rmd-->
...
Within our framework one can <del>simultaneosly</del> <mark>simultaneously</mark> test the importance of niche solely, neutrality solely and combinations of niche and neutral effects on community structure (abundance or occurrence). 
...
Main <del>crtiticism</del> <mark>criticism</mark> of our framework rely on: (1) Are fixed and random effects actually capturing niche and neutral dynamics? (2) Are random effects only capturing neutral dynamics or <del>overdipersion</del> <mark>overdispersion</mark> in data and uninformed species traits are inflating random effects? 

In order to show how we implemented our framework and how model components are translated into niche and neutral dynamics we performed simulations <mark>as describe</mark> below. We do not intend to scan all parameter space of type of communities and traits, neither make a comprehensive investigation of sampling <del>effetcs</del> <mark>effects</mark>. Our goal with this tutorial is to <del>adress</del><mark>address</mark> main criticisms of our framework and show how promising our framework is to quantify the relative importance of niche and neutral dynamics.

## A test of our modelling framework

Here, we present a working example of our modelling <del>framwork</del> <mark>framework</del>. We use simulated communities based on niche and neutral dynamics in order to show how fixed and random effects capture different ecological processes. Also, we use strong and week traits in order to show how fixed and random effects capture neutral dynamics when traits are strong. Therefore, we simulated stochastic and deterministic metacommunities and use strong or weak traits in model selection. 

We simulated metacommunities with the same data structure as <del>as</del> our abundance data of ferns in three mountain chains in southern Brazil. Then, we make poisson and negative binomial samples of communities and use samples from the metacommunities in our model selection framework.

In order to <del>acess</del> <mark>access</mark>main criticisms of our framework, we look into four scenarios:
...
**The simulations are based on several packages that should be installed in user computer before start. The above packages should be installed as regular CRAN packages before loaded. To check if they are already installed run the above code and installed if not, run the above code.**

```{r,  message=FALSE}

ipackages <- c("devtools", "ade4", "vegan", "lme4", "bbmle", "optimx", "piecewiseSEM", "sads", "ggplot2", "dplyr", "MASS", "knitr")
...
**Attached all packages needed running above code.**
...
<b>To evaluate our methods we </b> first <del>, we generated</del> the <del>metacoomunity</del> <mark> metacommunity </mark> data. Part of the code used here is adapted from <b> <a href="http://rstudio-pubs-static.s3.amazonaws.com/159425_80725873417e42fdb13821c10a198281.html">  Sokol's MCSim tutorial </a> </b>. We created data from 30 sites across three mountain chains. We simulated abundance values for 20 different species.

For both deterministic and stochastic scenarios, we simulated 10 different metacommunities until timestep 100 and took mean values from the 100^th^ last timestep.
...
## Distances between regions is an order of magnitude distance between sites within regions
...
## Each set of 10 points at same x coordinate is labeled as a region
...
## Calculate niches for species: optimal values along the environmental variable
...
simulation_landscape_det <- MCSim::fn.make.landscape(
  site.coords = sites[,c("x", "y")],
  Ef = sites$env,
  m = 0.5,
  JM = JM)
## It seems that in R 3.5 it is necessary to convert the distance matrix from dataframe to a matrix
simulation_landscape_det$dist.mat <-  as.matrix(simulation_landscape_det$dist.mat)
## checking if the dist mat is what we think it is
table(as.matrix(dist(sites[, c("x", "y")], upper=TRUE, diag=1)) == simulation_landscape_det$dist.mat)
...
## FAZ SENTIDO A ABUNDANCIA SER FRACIONADA, NAO DEVERIA SER NUMEROS INTEIROS?? FIZ ISSO E VAI DAR PROBLEMA DEPOIS NO MODELO GAMMA DA AMOSTRA BINOMIAL NEGATIVA
...
table(simulation_landscape_sto$site.info == simulation_landscape_det$site.info)
## VC FALA QUE "REDUCING IMMIGRATION PARAMETER, ALLOWING LIMITED DISPERSAL"
## ISSO NAO FAZ SENTIDO. O PARAMETRO E' O MESMO DA COMUNIDADE DETERMINISTICA, NAO HA DIFERENCA EM NENHUM DOS PARAMETROS DE CONSTRUCAO DA METACOMUNIDADE. NO CENARIO ESTOCASTICO ESSE PARAMETRO E MUITO IMPORTANTE POIS QUANTO MAIS MISTURA MAIS AS COMUNIDADES LOCAIS SAO PARECIDAS, OU SEJA A MESMA ABUNDANCIA E ESPERADA PARA A METACOMUNIDADE E COMUNIDADE LOCAL JA QUE TODAS AS ESPECIES TEM A MESMA CAPACIDADE DE DISPERSAO 
...
DEVEMOS DEIXAR CLARO AS PREMISSAS DE CADA UM DOS CENARIOS DE AMOSTRAGEM. PARA QUE FIQUE CLARO QUE A BINOMIAL NEGATIVA ESTA DANDO CONTA DA SOBREDISPERSAO DOS DADOS QUE A POISSON NAO DA CONTA.
...
sto.nb <- rnbinom(n= length(sto$count), 
                  size=gamma.shape(glm((sto$count+0.001)   ~ 1, family=Gamma))$alpha,
                  mu=sto$count*0.5)
## AQUI TIVE QUE ADICIONAR UMA FRACAO PARA PODER USAR A GAMMA. NO SEU CODIGO ORIGINAL O SEU "SPLITING SPECIES IN 3 FRACTIONS" NAO ESTAVA INCLUINDO O ZERO...
...
First, we will address the question of overdispersion in data. In  order to examine overdispersion issues, we compare the results of the selected model from stochastic communities with right traits sampled from poisson and negative binomial distributions.   
