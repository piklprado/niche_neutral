...
r2.table <- function(model){
    ...
    best.null <- function(model) {
        ...
        onlyBars <- function(form) reformulate(sapply(findbars(form), function(x)  parens(deparse(x))), response=".")
    ...
    ## O termo abaixo é sigma^2_d de Nakagawa et al. 2017 Interface. Mas veja eqs 5.7-5.8 para correcao.
    s2d <- log(1 + 1/exp(as.numeric(fixef(m0)))) 
