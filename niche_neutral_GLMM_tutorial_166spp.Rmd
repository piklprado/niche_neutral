...
the code used here is adapted from <b> <a
href="http://rstudio-pubs-static.s3.amazonaws.com/159425_80725873417e42fdb13821c10a198281.html">
Sokol's MCSim tutorial </a> </b>. 
To replicate our fern data set, we simulated a metaccomunity of 166 species
over in 10 in each of three regions (mountain chains). 
...
##         title="Deterministic community from Poisson sample")+
...
##     facet_wrap(~region) +
##     labs(x="Environmental gradient", y="Species abundance",
##          title="Stochastic community from Poisson sample") +
##     theme(legend.position="none")
all.data %>%
    filter(spp%in%index) %>%
...
example. We create a function for each of our hypothesis.
...
null.sto <- m.null(ab=all.data$sto, 
                    ...
                    spp=all.data$sp, family="poisson")
...
#Calculating R2
## r2.det.rt ## Comentei esta linha, pq só mostra o objeto. Tem algum problema?
## PI: se o neu.det passar a incluir o termo 1|site acho que pode usar ele como modelo nulo para este R2, no argumento 'null.model', não?
## Control arguments to avoid convergence issues in the fit of the neutral model
r2.det.wt <- r2.full2(envneu.det,
                      control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=5e7)) 
                      )$R2
...
marginal $R^{2}$ because of the importance of the term (1|spp:site)
and minimizes the influence of the trait-environment correlation. When
